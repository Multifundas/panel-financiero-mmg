...
function getManualHTML() {
  ...
  +     '<span><strong>Panel de Resumen:</strong> Al inicio del Dashboard aparece un panel con saludo, acciones '
  +     'pendientes (plantillas recurrentes por aplicar, inversiones por vencer), resumen de la ultima semana '
  +     '(ingresos, gastos, comparativa con la semana anterior), y recordatorios utiles (respaldos pendientes, '
  +     'cuentas inactivas). Puedes cerrarlo con la "X" y reaparecera al volver a iniciar sesion.</span>'
  +   '</div>'
  + '</div>'
  ...
  +     '<span><strong>Cargar Estado de Cuenta PDF:</strong> Puedes importar movimientos directamente desde '
  +     'el estado de cuenta de tu banco en formato PDF. El sistema extrae los movimientos, los clasifica '
  +     'automaticamente por categoria segun el concepto, y te permite revisar, editar la clasificacion y '
  +     'eliminar filas duplicadas antes de confirmar la importacion.</span>'
  +   '</div>'
  + '</div>'

  // ── CARGAR PDF ──
  + '<div class="manual-section">'
  +   '<h3><i class="fas fa-file-pdf" style="color:#ef4444;"></i> Cargar Estado de Cuenta (PDF)</h3>'
  +   '<p>Esta herramienta te permite importar movimientos desde el estado de cuenta de tu banco en formato PDF:</p>'
  +   '<ul>'
  +     '<li><strong>Paso 1 - Seleccionar cuenta:</strong> Elige a que cuenta pertenece el estado de cuenta.</li>'
  +     '<li><strong>Paso 2 - Subir PDF:</strong> Selecciona el archivo PDF de tu estado de cuenta bancario.</li>'
  +     '<li><strong>Paso 3 - Revision:</strong> El sistema extrae y clasifica automaticamente los movimientos. '
  +     'Se muestra una tabla con: Fecha, Concepto, Monto, Tipo (Ingreso/Gasto) y Categoria.</li>'
  +     '<li><strong>Paso 4 - Editar:</strong> Puedes cambiar la categoria de cualquier movimiento usando el '
  +     'menu desplegable. Si un movimiento ya lo habias capturado, seleccionalo con el checkbox y eliminalo.</li>'
  +     '<li><strong>Paso 5 - Confirmar:</strong> Haz clic en "Confirmar e Importar" para guardar todos los '
  +     'movimientos y actualizar los saldos automaticamente.</li>'
  +   '</ul>'
  +   '<p><strong>Clasificacion automatica:</strong> El sistema reconoce conceptos comunes y los clasifica '
  +   'en categorias como Alimentacion (OXXO, Walmart, Soriana), Transporte (Uber, gasolina, casetas), '
  +   'Vivienda (CFE, Telmex, agua), Salud (farmacias, hospitales), Entretenimiento (Netflix, Spotify), '
  +   'Impuestos (SAT, tenencia) y Familia (colegios, escuelas).</p>'
  +   '<div class="manual-tip">'
  +     '<i class="fas fa-lightbulb"></i> '
  +     '<span><strong>Consejo:</strong> Revisa bien los movimientos antes de confirmar. Usa los checkboxes para '
  +     'seleccionar y eliminar filas que ya hayas capturado manualmente y asi evitar duplicados.</span>'
  +   '</div>'
  + '</div>'
  ...
  +     '<li><strong>Registra movimientos:</strong> Ve anotando ingresos y gastos conforme ocurran. Tambien puedes importar tu estado de cuenta en PDF para cargar muchos movimientos de golpe.</li>'
  ...
  +   '<p>Panel Financiero MMG &mdash; Version 1.1</p>'
  + '</div>';
}
