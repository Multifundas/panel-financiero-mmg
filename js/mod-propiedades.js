    ...
    '<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:16px;margin-bottom:24px;">' +
      ...
      '<div class="card" style="border-left:3px solid var(--accent-green);cursor:pointer;" onclick="mostrarDesglosePropKPI(\'valor\')">' +
        '<div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">' +
          ...
          '</div>' +
          '<span style="font-size:16px;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Valor Portafolio</span>' +
        '</div>' +
      ...
      '<div class="card" style="border-left:3px solid var(--accent-red);cursor:pointer;" onclick="mostrarDesglosePropKPI(\'deuda\')">' +
        '<div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">' +
          ...
          '</div>' +
          '<span style="font-size:16px;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Deuda Total</span>' +
        '</div>' +
      ...
      '<div class="card" style="border-left:3px solid var(--accent-blue);cursor:pointer;" onclick="mostrarDesglosePropKPI(\'rentas\')">' +
        '<div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">' +
          ...
          '</div>' +
          '<span style="font-size:16px;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Rentas Mensuales</span>' +
        '</div>' +
      ...
      '<div class="card" style="border-left:3px solid var(--accent-amber);cursor:pointer;" onclick="mostrarDesglosePropKPI(\'preventas\')">' +
        '<div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">' +
          ...
          '</div>' +
          '<span style="font-size:16px;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Preventas</span>' +
        '</div>' +
      ...
      '<div class="card" style="border-left:3px solid var(--accent-purple);cursor:pointer;" onclick="mostrarDesglosePropKPI(\'terminadas\')">' +
        '<div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">' +
          ...
          '</div>' +
          '<span style="font-size:16px;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Terminadas</span>' +
        '</div>' +
...
function filterPropiedades() {
  ...
  container.innerHTML = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">' +
    ...
    var actions =
      '<button class="btn btn-secondary" style="padding:4px 8px;font-size:16px;margin-right:4px;" onclick="editPropiedad(\'' + p.id + '\')" title="Editar"><i class="fas fa-edit"></i></button>';
    if (p.tipo === 'preventa') {
      actions += '<button class="btn btn-primary" style="padding:4px 8px;font-size:16px;margin-right:4px;" onclick="registrarPagoMensualidad(\'' + p.id + '\')" title="Registrar Pago"><i class="fas fa-money-bill-wave"></i></button>';
    } else {
      actions += '<button class="btn btn-primary" style="padding:4px 8px;font-size:16px;margin-right:4px;" onclick="actualizarValor(\'' + p.id + '\')" title="Actualizar Valor"><i class="fas fa-chart-line"></i></button>';
    }
    actions += '<button class="btn btn-danger" style="padding:4px 8px;font-size:16px;" onclick="deletePropiedad(\'' + p.id + '\')" title="Eliminar"><i class="fas fa-trash"></i></button>';
...
function buildPreventaCard(p, moneda) {
    ...
    if (diffDias > 0) {
      ...
      countdownHTML =
        '<div style="margin-top:12px;padding:10px 12px;border-radius:8px;background:var(--accent-amber-soft);display:flex;align-items:center;gap:8px;">' +
          ...
          '<span style="font-size:17px;color:var(--accent-amber);font-weight:600;">Entrega en ' + parts.join(', ') + ' (' + formatDate(p.fecha_entrega) + ')</span>' +
        '</div>';
    } else if (diffDias === 0) {
      countdownHTML =
        '<div style="margin-top:12px;padding:10px 12px;border-radius:8px;background:var(--accent-green-soft);display:flex;align-items:center;gap:8px;">' +
          ...
          '<span style="font-size:17px;color:var(--accent-green);font-weight:600;">Entrega programada para hoy</span>' +
        '</div>';
    } else {
      countdownHTML =
        '<div style="margin-top:12px;padding:10px 12px;border-radius:8px;background:var(--accent-red-soft);display:flex;align-items:center;gap:8px;">' +
          ...
          '<span style="font-size:17px;color:var(--accent-red);font-weight:600;">Entrega vencida hace ' + Math.abs(diffDias) + ' dias (' + formatDate(p.fecha_entrega) + ')</span>' +
        '</div>';
    }
  }
  ...
  return (
    ...
    '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:16px;">' +
      '<div>' +
        '<div style="font-size:16px;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Enganche</div>' +
        '<div style="font-size:19px;font-weight:700;color:var(--accent-blue);">' + formatCurrencyInt(p.enganche || 0, moneda) + '</div>' +
      '</div>' +
      '<div>' +
        '<div style="font-size:16px;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Total Pagado</div>' +
        '<div style="font-size:19px;font-weight:700;color:var(--accent-green);">' + formatCurrencyInt(res.total_pagado, moneda) + '</div>' +
      '</div>' +
      '<div>' +
        '<div style="font-size:16px;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Saldo Pendiente</div>' +
    ...
    '<div style="margin-bottom:12px;">' +
      '<div style="display:flex;justify-content:space-between;margin-bottom:4px;">' +
        '<span style="font-size:16px;color:var(--text-muted);">Progreso de Pagos</span>' +
        '<span style="font-size:16px;font-weight:600;color:var(--text-primary);">' + pagadas + ' / ' + totales + ' mensualidades</span>' +
      '</div>' +
    ...
    /* Details row */
    '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;font-size:17px;color:var(--text-muted);">' +
...
function buildTerminadaCard(p, moneda) {
  ...
  return (
    ...
    '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:16px;">' +
      '<div>' +
        '<div style="font-size:16px;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Valor de Mercado Estimado</div>' +
        '<div style="font-size:19px;font-weight:700;color:var(--accent-blue);">' + formatCurrencyInt(p.valor_actual, moneda) + '</div>' +
      '</div>' +
      '<div>' +
        '<div style="font-size:16px;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Renta Mensual</div>' +
        '<div style="font-size:19px;font-weight:700;color:var(--accent-green);">' + formatCurrencyInt(p.renta_mensual || 0, moneda) + '</div>' +
      '</div>' +
      '<div>' +
        '<div style="font-size:16px;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;">Rendimiento Anual</div>' +
  ...
  var formHTML =
    '<form id="formPropiedad" onsubmit="savePropiedad(event)">' +
      ...
      '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;">' +
        ...
        '<div class="form-group"><label class="form-label">Valor de Mercado Estimado *</label><div style="font-size:14px;color:var(--text-muted);margin-top:-4px;margin-bottom:4px;">Precio estimado de venta hoy en el mercado</div>' +
      ...
      '<div id="preventaFields" style="display:' + (tipoVal === 'preventa' ? 'block' : 'none') + ';">' +
        '<div style="padding:12px;border-radius:8px;background:var(--bg-base);margin-bottom:12px;">' +
          '<div style="font-size:17px;font-weight:600;color:var(--accent-amber);margin-bottom:8px;">Datos de Preventa</div>' +
          '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">' +
          ...
          '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;">' +
            ...
            '<div class="form-group"><label class="form-label">Monto Pago Final</label><div style="font-size:14px;color:var(--text-muted);margin-top:-4px;margin-bottom:4px;">Pago al termino del acuerdo</div>' +
      ...
      '<div id="terminadaFields" style="display:' + (tipoVal === 'terminada' ? 'block' : 'none') + ';">' +
        '<div style="padding:12px;border-radius:8px;background:var(--bg-base);margin-bottom:12px;">' +
          '<div style="font-size:17px;font-weight:600;color:var(--accent-purple);margin-bottom:8px;">Datos de Propiedad Terminada</div>' +
          '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">' +
...
function savePropiedad(event) {
  ...
  if (tipo === 'preventa') {
    ...
    data.monto_pago_final = parseFloat(document.getElementById('propiedadMontoPagoFinal').value) || 0;
    data.fecha_pago_final = document.getElementById('propiedadFechaPagoFinal').value;
  ...
  var formHTML =
    '<form id="formActualizarValor" onsubmit="saveActualizarValor(event, \'' + propiedadId + '\')">' +
      '<div style="margin-bottom:16px;padding:12px;border-radius:8px;background:var(--bg-base);">' +
        '<div style="font-size:17px;color:var(--text-muted);margin-bottom:4px;">Propiedad: <strong style="color:var(--text-primary);">' + prop.nombre + '</strong></div>' +
        '<div style="font-size:17px;color:var(--text-muted);">Valor de mercado actual: <strong style="color:var(--accent-blue);">' + formatCurrencyInt(prop.valor_actual, prop.moneda || 'MXN') + '</strong></div>' +
      '</div>' +
...
function verCalendarioPagos() {
  ...
  propiedades.forEach(function (p) {
    ...
    if (p.tipo === 'preventa') {
      ...
      /* Pago final event */
      if (p.monto_pago_final > 0 && p.fecha_pago_final) {
        var pfDate = new Date(p.fecha_pago_final + 'T00:00:00');
        ...
        for (var pf = 0; pf < 3; pf++) {
          if (pfMonth === meses3[pf].mes && pfYear === meses3[pf].anio) {
            eventos.push({
              mesIdx: pf,
              tipo: 'pago_final',
              nombre: p.nombre + ' (Pago Final)',
              monto: p.monto_pago_final,
              montoMXN: toMXN(p.monto_pago_final, moneda, tiposCambio),
              moneda: moneda,
              fecha: pfDate,
              dia: pfDay
            });
  ...
  if (proximosPagos.length > 0) {
    proximoPagoHTML = proximosPagos.map(function(pp) {
      var vencidoBadge = pp.vencido ? ' <span style="font-size:13px;background:var(--accent-red);color:#fff;padding:1px 5px;border-radius:4px;font-weight:600;">VENCIDO</span>' : '';
      return '<div style="margin-bottom:4px;">' +
        '<div style="font-size:17px;font-weight:700;color:var(--accent-red);">' + formatCurrencyInt(pp.monto, pp.moneda) + vencidoBadge + '</div>' +
        '<div style="font-size:14px;color:var(--text-muted);">' + pp.nombre + ' - ' + formatDate(pp.fecha) + '</div>' +
      '</div>';
    }).join('');
  } else {
    proximoPagoHTML = '<div style="font-size:17px;color:var(--text-muted);">Sin pagos pendientes</div>';
  }
    ...
    html +=
      '<div style="display:flex;flex-direction:column;">' +
        ...
        '<div style="padding:10px 12px;border-radius:8px;background:' + (isCurrent ? 'var(--accent-blue)' : 'var(--bg-base)') + ';margin-bottom:12px;text-align:center;">' +
          ...
          (isCurrent ? '<div style="font-size:14px;color:rgba(255,255,255,0.7);margin-top:2px;">Mes actual</div>' : '') +
        '</div>';

    if (mesEventos.length === 0) {
      html +=
        '<div style="text-align:center;padding:20px 8px;color:var(--text-muted);font-size:17px;">' +
      ...
      mesEventos.forEach(function (ev) {
        ...
        if (ev.tipo === 'pago_final') {
          bgColor = 'rgba(139,92,246,0.12)';
          borderColor = 'var(--accent-purple)';
          iconClass = 'fa-landmark';
          iconColor = 'var(--accent-purple)';
          labelTipo = 'Pago Final';
        } else if (ev.tipo === 'mensualidad') {
        ...
        var montoColor = ev.tipo === 'renta' ? 'var(--accent-green)' : (ev.tipo === 'pago_final' ? 'var(--accent-purple)' : (ev.tipo === 'ultima_mensualidad' ? 'var(--accent-purple)' : (ev.tipo === 'saldo_remanente' ? 'var(--accent-amber)' : (ev.tipo === 'mensualidad' ? 'var(--accent-red)' : 'var(--accent-amber)'))));
        var signo = ev.tipo === 'renta' ? '+' : '-';

        var extraInfo = '';
        if (ev.tipo === 'mensualidad' && ev.restantes !== undefined) {
          extraInfo = '<div style="font-size:14px;color:var(--text-muted);margin-top:2px;">' + ev.restantes + ' restante' + (ev.restantes !== 1 ? 's' : '') + '</div>';
        }
          ...
          '<div style="display:flex;align-items:flex-start;gap:10px;padding:10px;margin-bottom:8px;border-radius:8px;background:' + bgColor + ';border-left:3px solid ' + borderColor + ';">' +
            ...
            '<div style="flex:1;min-width:0;">' +
              '<div style="display:flex;justify-content:space-between;align-items:baseline;gap:4px;">' +
                '<div style="font-size:16px;font-weight:600;color:var(--text-primary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="' + ev.nombre + '">' + ev.nombre + '</div>' +
                '<div style="font-size:14px;color:var(--text-muted);white-space:nowrap;">Dia ' + ev.dia + '</div>' +
              '</div>' +
              '<div style="font-size:17px;font-weight:700;color:' + montoColor + ';margin-top:2px;">' + signo + ' ' + formatCurrencyInt(ev.monto, ev.moneda) + '</div>' +
              '<div style="font-size:14px;color:var(--text-muted);margin-top:1px;">' + labelTipo + '</div>' +
              extraInfo +
            '</div>' +
          '</div>';
      });
    ...
    if (mesEventos.length > 0) {
      html +=
        '<div style="margin-top:auto;padding:8px 10px;border-radius:6px;background:var(--bg-base);">' +
          ...
          '</div>' +
          '<div style="display:flex;justify-content:space-between;font-size:16px;font-weight:700;color:' + mesNetColor + ';border-top:1px solid var(--border-color);padding-top:4px;">' +
            '<span>Neto</span>' +
  ...
  html +=
    '<div style="display:flex;align-items:center;gap:16px;margin-top:12px;padding:8px 12px;">' +
      '<div style="display:flex;align-items:center;gap:5px;">' +
        ...
        '<span style="font-size:16px;color:var(--text-muted);">Mensualidad</span>' +
      '</div>' +
      '<div style="display:flex;align-items:center;gap:5px;">' +
        '<div style="width:10px;height:10px;border-radius:2px;background:var(--accent-purple);"></div>' +
        '<span style="font-size:16px;color:var(--text-muted);">Ultima Mensualidad</span>' +
      '</div>' +
      '<div style="display:flex;align-items:center;gap:5px;">' +
        '<div style="width:10px;height:10px;border-radius:2px;background:var(--accent-green);"></div>' +
        '<span style="font-size:16px;color:var(--text-muted);">Renta</span>' +
      '</div>' +
      '<div style="display:flex;align-items:center;gap:5px;">' +
        '<div style="width:10px;height:10px;border-radius:2px;background:var(--accent-amber);"></div>' +
        '<span style="font-size:16px;color:var(--text-muted);">Mantenimiento / Saldo Remanente</span>' +
      '</div>' +
      '<div style="display:flex;align-items:center;gap:5px;">' +
        '<div style="width:10px;height:10px;border-radius:2px;background:var(--accent-purple);"></div>' +
        '<span style="font-size:16px;color:var(--text-muted);">Pago Final</span>' +
      '</div>' +
    '</div>';
  ...
  if (preventaResumen.length > 0) {
    html +=
      '<div class="card" style="margin-top:16px;">' +
        ...
        '<div style="overflow-x:auto;"><table class="data-table sortable-table" style="font-size:17px;"><thead><tr>' +
    ...
    preventaResumen.forEach(function(pr) {
      ...
      var pfLabel = pr.tienePagoFinal
        ...
        : (pr.saldoRemanente > 0 ? formatCurrencyInt(pr.saldoRemanente, pr.moneda) + ' <span class="badge badge-amber" style="font-size:13px;">Estimado</span>' : formatCurrencyInt(0, pr.moneda) + ' <span class="badge badge-green" style="font-size:13px;">Cubierto</span>');
...
function mostrarDesglosePropKPI(tipo) {
  ...
  if (tipo === 'valor') {
    ...
    var preventasP = propiedades.filter(function(p) { return p.tipo === 'preventa'; });
    if (preventasP.length === 0) { openModal(titulo, '<p style="text-align:center;padding:20px;color:var(--text-muted);">No hay propiedades en preventa.</p>'); return; }
    rows = preventasP.map(function(p) {
      var moneda = p.moneda || 'MXN';
      var res = calcularResumenPreventa(p);
      var pagadas = p.mensualidades_pagadas || 0;
      var totales = p.mensualidades_total || 0;
      ...
      var pagoFinalHTML = p.monto_pago_final ? formatCurrencyInt(p.monto_pago_final, moneda) + (p.fecha_pago_final ? '<br><span style="font-size:14px;color:var(--text-muted);">' + formatDate(p.fecha_pago_final) + '</span>' : '') : '-';
      return '<tr>' +
        '<td style="font-weight:600;color:var(--text-primary);">' + p.nombre + '<br><span style="font-size:14px;color:var(--text-muted);">' + (p.ubicacion || '') + '</span></td>' +
    ...
    var terminadasT = propiedades.filter(function(p) { return p.tipo === 'terminada'; });
    if (terminadasT.length === 0) { openModal(titulo, '<p style="text-align:center;padding:20px;color:var(--text-muted);">No hay propiedades terminadas.</p>'); return; }
    rows = terminadasT.map(function(p) {
      var moneda = p.moneda || 'MXN';
      var res = calcularResumenTerminada(p);
